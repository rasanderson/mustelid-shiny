...
class ImageFolder(Dataset):
    ...
    def __init__(self, image_dir, transform=None, path_head=None):
        """
        ...
        self.image_dir = os.path.join(path_head, image_dir) if path_head else image_dir
        self.transform = transform
        self.path_head = path_head
        self.images = [os.path.join(dp, f) for dp, dn, filenames in os.walk(self.image_dir) for f in filenames if is_image_file(f)] # dp: directory path, dn: directory name, f: filename
...
class ClassificationImageFolder(ImageFolder):
    ...
    def __init__(self, image_dir, transform=None, path_head=None):
        ...
        Parameters:
            ...
            transform (callable, optional): Optional transform to be applied on the image.
        """
        super(ClassificationImageFolder, self).__init__(image_dir, transform, path_head)
...
class DetectionCrops(Dataset):
    ...
    def __getitem__(self, idx) -> tuple:
        ...
        try:
            ...
            raise ImportError(
                "supervision is required to use DetectionCrops. Install supervision to load detection crops."
            ) from exc
